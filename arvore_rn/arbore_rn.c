...
No* rotaciona_para_esquerda(No *p){
    No* x = p->dir;
    p->dir = x->esq;
    x->esq = p;
    x->cor = p->cor;
...
No* rotaciona_para_direita(No *p){
    No* x = p->esq;
    p->esq = x->dir;
    x->dir = p;
    x->cor = p->cor;
...
No *inserir_rec(No *p, int chave){
    ...
    if (chave < p->chave)
        p->esq = inserir_rec(p->esq, chave);
    else
	    p->dir = inserir_rec(p->dir, chave);
    return p;
}

No* inserir(No *raiz, int chave){
    raiz = inserir_corrigido(raiz, chave);
    raiz->cor = PRETO;
    return raiz;
